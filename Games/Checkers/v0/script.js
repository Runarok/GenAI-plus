(function(){
  ...
  function render(){
    ...
    turnLabel.className = currentPlayer==='red'?'text-red-600 font-medium':'text-blue-600 font-medium';
    ...
    board.forEach((row,r)=>row.forEach((pc,c)=>{
      ...
      if(pc){
        ...
        div.className = `piece-hover w-8 h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center shadow-lg transition-all duration-200 border-2 ${
          pc.player==='red' ? 'bg-gradient-to-br from-red-400 to-red-600 border-red-700'
                           : 'bg-gradient-to-br from-blue-400 to-blue-600 border-blue-700'
        }`;
        if(pc.type==='king') div.classList.add('ring-2','ring-yellow-400');
        div.textContent = pc.type==='king' ? '👑' : '●';
        sq.appendChild(div);
