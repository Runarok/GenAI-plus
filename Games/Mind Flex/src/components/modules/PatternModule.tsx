...
const PatternModule: React.FC<PatternModuleProps> = ({ timeRemaining }) => {
  ...
  useEffect(() => {
    const patternLength = Math.min(9, moduleLevel + (difficulty === 'easy' ? 1 : difficulty === 'medium' ? 2 : 3));
    ...
    // Show pattern for a few seconds based on difficulty
    const showTime = 
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-sm mx-auto">
      ...
      <div className="text-center text-slate-300">
        {isShowingPattern ? (
          <p>Memorize the highlighted squares</p>
        ) : (
          <p>Recreate the pattern by tapping the squares</p>
        )}
      </div>
    </div>
  );
