...
class GameManager {
    constructor() {
        ...
        this.JUMP_FORCE = 11;
        this.MOVE_SPEED = 4.5;
        this.AIR_CONTROL = 0.25;
        this.DOT_RADIUS = 12;
        this.JUMP_DISTANCE = 130;
        this.LEVEL_HEIGHT = 100;
        this.CAMERA_SMOOTHING = 0.06;
        this.PLAYER_SMOOTHING = 0.12;
    ...
    generateDots() {
        ...
        const endY = -5000; // Generate dots up to 5000px above
        ...
        while (currentY > endY) {
            ...
            this.gameState.difficulty = Math.min(level * 0.1 + 1, 2);
            
            // Generate 2-3 dots per level
            const dotsThisLevel = Math.floor(Math.random() * 2) + 2;
            
            for (let i = 0; i < dotsThisLevel; i++) {
                const minDistance = 60;
                const maxDistance = Math.min(110, 60 + level * 3);
                const distance = Math.random() * (maxDistance - minDistance) + minDistance;
                
                // Limit angle to ensure reachability
                const maxAngle = Math.min(Math.PI * 0.5, Math.PI * 0.3 + level * 0.01);
                const angle = (Math.random() - 0.5) * maxAngle;
                
                let nextX = currentX + Math.sin(angle) * distance;
                const nextY = currentY - Math.cos(angle) * distance * 0.7;
                
                // Keep dots within center band
    ...
    updatePlayer() {
        ...
        // Apply movement with different responsiveness
        ...
        // Jump handling
        const canJump = player.onGround || (Date.now() - player.lastGroundTime < 150); // Coyote time
        if (window.controlsManager && window.controlsManager.isJumping() && canJump) {
    ...
    updateDots() {
        this.gameState.dots.forEach(dot => {
            if (dot.type === this.DOT_TYPES.MOVING) {
                dot.x += dot.moveDirection * 0.4;
                if (Math.abs(dot.x - dot.originalX) > 40) {
                    dot.moveDirection *= -1;
                }
            }
        });
    ...
    updateCamera() {
        ...
        this.gameState.camera.targetY = player.y - rect.height * 0.6;
