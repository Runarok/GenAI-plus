...
class GameManager {
    constructor() {
        ...
        // Improved physics constants for smoother gameplay
        this.GRAVITY = 0.28;           // Reduced from 0.35
        this.JUMP_FORCE = 9.5;        // Reduced from 11
        this.MOVE_SPEED = 3.5;        // Reduced from 4.5
        this.AIR_CONTROL = 0.15;      // Reduced from 0.25
        this.DOT_RADIUS = 12;
        this.JUMP_DISTANCE = 140;     // Increased reach
        this.LEVEL_HEIGHT = 120;      // Increased spacing
        this.CAMERA_SMOOTHING = 0.04; // Slower camera
        this.PLAYER_SMOOTHING = 0.08; // Slower movement
        this.VELOCITY_DAMPING = 0.95; // Prevent drift
    ...
    generateDots() {
        ...
        // Define climbing band - narrower for better spacing
        const centerBand = {
            left: rect.width * 0.3,
        ...
        while (currentY > endY) {
            ...
            // Generate 2-3 dots per level with better spacing
            ...
            for (let i = 0; i < dotsThisLevel; i++) {
                ...
                const maxDistance = Math.min(130, 90 + level * 2);
                const distance = Math.random() * (maxDistance - minDistance) + minDistance;
                
                // Limit angle for reachability
                const maxAngle = Math.min(Math.PI * 0.4, Math.PI * 0.25 + level * 0.008);
    ...
    updatePlayer() {
        ...
        if (isJumpPressed && !player.jumpPressed && canJump) {
            ...
            player.momentum = player.vx * 0.15; // Reduced momentum transfer
            this.playSound('jump');
        }
    ...
    checkCollisions() {
        ...
        for (let i = this.gameState.dots.length - 1; i >= 0; i--) {
            ...
            if (distance < player.radius + this.DOT_RADIUS && player.vy > 0) {
                ...
                player.vy = 0;
                player.vx *= 0.8; // Reduce horizontal velocity on landing
